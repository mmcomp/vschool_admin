              ...
              <div class="col-xs-12">
                  <div class="box">
                      <div class="box-header">
                          ...
                          <a class="btn btn-primary pull-left" href="/sign/create">ثبت نشان</a>
                      </div><!-- /.box-header -->
                      <div class="box-body">
                          <table id="example2" class="table table-bordered table-hover table-striped" data-page-length='20'>
                              ...
                              @foreach($signs as $i=>$sign)
                                <tr>
                                  <td>{{ $i + 1 }}</td>
                                  <td>{{ $sign->name }}</td>
                                  <td>{{ $sign->pstart_time }}</td>
                                  <td>{{ $sign->pend_time }}</td>
                                  <td>
                                  @if($sign->image_path && $sign->image_path!='')
                                    <img src="{{ $sign->image_path }}" style="height: 30px;" />
                                  @else
                                    -
                                  @endif
                                  </td>
                                  <td>
                                    <a class="btn btn-success" href="/sign/edit/{{ $sign->id }}">
                                    ویرایش
                                    </a>
                                    <a class="btn btn-danger btn-delete" href="/sign/delete/{{ $sign->id }}">
                                    حذف
                                    </a>
                                  </td>
                                </tr>
